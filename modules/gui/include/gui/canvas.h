...
public:
    ...
    void FillPath() {
        // Triangle fan from first point (works for convex polygons)
        if (m_pathPoints.size() < 3) return;
        auto p0 = P(m_pathPoints[0].x, m_pathPoints[0].y);
        for (size_t i = 1; i + 1 < m_pathPoints.size(); i++) {
            auto p1 = P(m_pathPoints[i].x, m_pathPoints[i].y);
            auto p2 = P(m_pathPoints[i + 1].x, m_pathPoints[i + 1].y);
            m_renderer.DrawTriangle(p0, p1, p2, m_state.fillColor);
        }
    }
